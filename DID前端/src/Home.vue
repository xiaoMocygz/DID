...
import md5 from 'md5'
import { userinfo } from './utils/home';
  ...
  async created() {
    ...
    if (address == false || address == '') {
      if (this.$cookie.get('token')) {
        var {
          data: { code, message }
        } = await login({ email: this.$cookie.get('email'), password: md5(this.$cookie.get('password')) })
        if (code == 1) return this.$switch(message, '邮箱格式错误!', '邮箱未注册!', '密码错误!', '钱包地址错误!', '登录错误!')
        this.$Toast.success('登录成功')
        const res = await userinfo()
        var { code: Code, items } = res.data
        if (Code == 0) {
          this.items = items
        }
      } else {
        this.$Toast('3秒后自动跳转到登录')
        setTimeout(() => {
    ...
    this.$bus.$emit('telegramClick',this.items.telegram
 //  return this.items.telegram || false
	)
    //loadweb3(userBaseMes);
  },
  methods: {
    authTypeClick() {
      return this.items.authType || false
    },
