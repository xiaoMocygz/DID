	...
	<div>
		...
		<div class="content">
				...
				<div class="currencyType">
					<div class="addCurrencyBtn">
						<van-button @click="handleClick" class="btn" size="small">
							<CurCurrencyType @show-currency-type="handleClick" :showCurrencyType="showCurrencyType" :payType="currencyId"></CurCurrencyType>
						</van-button>
					</div>
					<div class="addPayBtn">
			...
			<!-- <van-swipe-cell v-for="(receiving, index) of receivingList" :border="false" :key="index">
					<van-cell>
						<template #icon>
							<img class="pay-img" :src="require('@/static/image/' + receiving.icon + '.png')" alt="aug-icon" />
						</template>
						<template #title>
						...
						<template #right-icon>
							<van-icon name="replay" @click="addpayFlgOpen(index, receiving.title, receiving.account)" size="22px" />
						</template>
					</van-cell>
		...
		<!-- 选择当前交易货币类型弹窗 -->
		<van-popup v-model="showCurrencyType" round position="bottom" :style="{ height: '85%' }">
			<div class="searchType">
				<van-form @submit="onSearch" validate-trigger="onChange" ref="CurrencyTypeipt" class="ipt">
					<van-field name="moneyTtype" v-model.trim.lazy="moneyTtype" validate-trigger="onBlur" placeholder="搜索" :rules="currencyRules" />
				</van-form>
				...
				<div class="currencyList">
					...
					<div class="currencyContent" v-for="imgnAme in searchStatus ? seachcountrie : countrieImgs" :key="imgnAme" @click="changeCurID(imgnAme)">
						<!-- 注意 图片路径  -->
		...
		<van-popup class="pop" position="bottom" round v-model="addpayFlg">
			<van-picker :default-index="defaultIndex" show-toolbar :columns="addPaymentList" @confirm="onConfirm" @cancel="onCancel" @change="onChange" />
		</van-popup>
	</div>
</template>
