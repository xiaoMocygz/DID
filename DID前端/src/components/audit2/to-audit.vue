		...
		<van-popup round v-model="show" position="bottom" :style="{ height: 'auto' }">
			...
			<div class="xia">
				<div class="type">
					<span>具体原因</span>
					<div class="item">
						<span v-show="!show1">输入</span> <span v-show="show1">切换</span
						><van-icon name="search" size="20px" color="#237ff8" class="iconfont icon-qiehuan" class-prefix="icon" @click="show1 = !show1" />
					</div>
				</div>
				<div class="reason" v-show="show1">
					<div class="item" v-for="(item, index) in reason" :class="item.flag ? 'check' : ''" :key="index" @click="reasonCheckd(item)">
						{{ item.title }}
					</div>
				</div>
				<div class="textarea" v-show="!show1">
