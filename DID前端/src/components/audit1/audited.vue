		...
		<div class="not_content">
			<div v-if="list.length != 0">
				<div class="content" v-for="(item, index) in list" :key="index">
					...
					<p>证件及手持证件照</p>
					<div class="imgs">
						<van-image class="left" :src="item.Img1" lazy-load @click="imgClick(item, 0)" fit="cover" alt="">
							<div></div>
						</van-image>
						<van-image :src="item.Img2" lazy-load @click="imgClick(item, 1)" fit="cover" alt=""> </van-image>
					</div>
					<van-image-preview v-model="imgShow" :images="images" :show-index="false">
						<template v-slot:cover>
							<div v-if="show" @click="imgShow = false" class="shade-left"></div>
						</template>
					</van-image-preview>
					<div class="time">
						<div v-if="item.Kyc != 2">
